...
class GetOrganisationResponse extends AbstractResponse
{
    ...
    public function getErrorMessage()
    {
        if ($this->data) {
            ...
            elseif(array_key_exists('errors', $this->data)) {
                return \PHPAccounting\MyobAccountRightLive\Helpers\Legacy\ErrorResponseHelper::parseErrorResponse($this->data['errors'][0]['message']);
            } else {
                if (array_key_exists('Items', $this->data)) {
                    if (count($this->data['Items']) == 0) {
    ...
    public function getOrganisations(){
        ...
        if (array_key_exists('items', $this->data)) {
            foreach ($this->data['items'] as $organisation) {
                $newOrganisation = [];
                $newOrganisation['accounting_id'] = \PHPAccounting\MyobAccountRightLive\Helpers\IndexSanityCheckHelper::indexSanityCheck('uid', $organisation);
                $newOrganisation['name'] = \PHPAccounting\MyobAccountRightLive\Helpers\IndexSanityCheckHelper::indexSanityCheck('name', $organisation);
                $newOrganisation['uri'] = \PHPAccounting\MyobAccountRightLive\Helpers\IndexSanityCheckHelper::indexSanityCheck('uri', $organisation);
                $newOrganisation['country_code'] = \PHPAccounting\MyobAccountRightLive\Helpers\IndexSanityCheckHelper::indexSanityCheck('country', $organisation);
                $newOrganisation['gst_registered'] = \PHPAccounting\MyobAccountRightLive\Helpers\IndexSanityCheckHelper::indexSanityCheck('gstRegistered', $organisation);
